...
LOTEDATA_ADDRESS_PREFIX = hashlib.sha512(
    FAMILY_NAME.encode('utf-8')).hexdigest()[0:6]


def make_lotedata_address(name):
    return LOTEDATA_ADDRESS_PREFIX + hashlib.sha512(
        name.encode('utf-8')).hexdigest()[-64:]


class LotedataTransactionHandler(TransactionHandler):
    ...
    @property
    def namespaces(self):
        return [LOTEDATA_ADDRESS_PREFIX]
    ...
    def apply(self, transaction, context):
        ...
        updated_state = _do_lotedata(verb, name, value, state)
...
def _get_state_data(name, context):
    address = make_lotedata_address(name)
...
def _do_lotedata(verb, name, value, state):
    verbs = {
        'set': _do_set,
        'inc': _do_inc,
        'dec': _do_dec,
