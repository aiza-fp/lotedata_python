...
from sawtooth_lotedata.client_cli.workload.workload_generator import \
    WorkloadGenerator
from sawtooth_lotedata.client_cli.workload.sawtooth_workload import Workload
from sawtooth_lotedata.client_cli.create_batch import create_lotedata_transaction
from sawtooth_lotedata.client_cli.create_batch import create_batch
...
LoteDataState = namedtuple('LoteDataState', ['name', 'url', 'value'])
...
class LoteDataWorkload(Workload):
    """
    ...
    def __init__(self, delegate, args):
        ...
        if args.key_file is not None:
            try:
                ...
                raise LoteDataCliException(str(pe)) from pe
            except IOError as ioe:
                raise LoteDataCliException(str(ioe)) from ioe
        else:
            self._signer = crypto_factory.new_signer(
    ...
    def on_batch_committed(self, batch_id):
        ...
        if key is not None:
            if key.value < 1000000:
                txn = create_lotedata_transaction(
                    verb="inc",
                    name=key.name,
                    value=1,
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        LoteDataState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and LoteDataWorkload. Set LoteData workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = LoteDataWorkload(generator, args)
