...
from sawtooth_lotedata.processor.handler import make_lotedata_address
...
def create_lotedata_transaction(verb, name, value, deps, signer):
    """Creates a signed lotedata transaction.
    ...
    Returns:
        transaction (transaction_pb2.Transaction): the signed lotedata
            transaction
    """
    payload = LoteDataPayload(
        verb=verb, name=name, value=value)

    # The prefix should eventually be looked up from the
    # validator's namespace registry.
    addr = make_lotedata_address(name)

    header = transaction_pb2.TransactionHeader(
        signer_public_key=signer.get_public_key().as_hex(),
        family_name='lotedata',
        family_version='1.0',
        inputs=[addr],
        outputs=[addr],
        dependencies=deps,
...
def add_create_batch_parser(subparsers, parent_parser):
    ...
    epilog = '''
    details:
     create sample batch(es) of lotedata transactions.
     populates state with lotedata key/value pairs
